...
def _machine(parser):
    parser.add_argument(
        'machine',
        help='machine on which the action will be performed')
...
def _router(parser):
    ...
    parser.add_argument(
        'router', nargs=nargs, default=default,
...
def _server(parser):
    ...
    nargs = '?' if default else None
    parser.add_argument(
        'server', nargs=nargs, default=default,
...
def _make(parser):
    parser.add_argument(
        '-e', '--extra', type=str,
        help='extra debian package(s) to install')
    parser.add_argument(
        '-n', '--name',
        type=str, help='name/tag to add to the build version')
    parser.add_argument(
        '-b', '--backdoor', type=str,
        help='install an admin account on the iso with this passord')
    parser.add_argument(
        '-f', '--force',
        help='make without custom package', action='store_true')
    parser.add_argument(
        '-t', '--test',
        help='test the iso when built', action='store_true')
...
def _repository(parser):
    parser.add_argument(
        'branch',
        help='the phabricator/branch to work on')
    parser.add_argument(
...
def _presentation(parser):
    ...
    parser.add_argument(
        ...
        help='do not show what is happening')
...
def _edit(parser):
    parser.add_argument(
        ...
        help='start editor once branched')
...
def _upgrade(parser):
    ...
    parser.add_argument(
        '-b', '--bind', type=int,
        help='ip to bind the webserver to')
    parser.add_argument(
        '-r', '--remote', type=int,
        help='port to bind the router')
    parser.add_argument(
        '-l', '--local', type=int, default=8088,
        help='port to bind the webserver')
